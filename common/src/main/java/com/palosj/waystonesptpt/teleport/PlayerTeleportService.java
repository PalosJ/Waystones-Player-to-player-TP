...
public final class PlayerTeleportService {
    ...
    static void handleRequest(
        ...
        try {
            ...
            WaystonesPTPT.LOGGER.warn("Waystones rejected a player-destination teleport before it started.", error);
    ...
    private static void settleTeleport(
            ...
            TeleportRuntimeBoundary runtime) {
        if (error != null) {
            WaystonesPTPT.LOGGER.warn("Waystones player-destination teleport failed asynchronously.", error);
            runtime.displayMessage(sender, "message.waystonesptpt.compatibility_unavailable");
            return;
        }
        if (result.isEmpty()) {
