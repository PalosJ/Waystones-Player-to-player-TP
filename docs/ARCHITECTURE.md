...
- Waystones 搜索框只过滤 Waystones；玩家搜索框只在当前 listed 玩家目录内按名字筛选，不改变完整接收状态订阅。搜索或名单改变才重新筛选，仍在线但被筛掉的 UUID 行与头像缓存保留。命名布局缩放保留查询；头像模式与关闭菜单清空。屏幕关闭、连接断开或屏幕替换时显式清理布局、控件和皮肤状态。
- 玩家工具行复用行宽：原生 20px 接收按钮与搜索框相隔 2px，整体与玩家行按钮中心一致；头像模式只保留 20px 开关。图标依据服务端确认状态绘制，等待期间灰色禁用。输入框聚焦时消费容器快捷键与 Enter，Tab/Esc 交回屏幕处理。
