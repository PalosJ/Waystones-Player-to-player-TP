...
26.x 使用 Java 25，并把 Shogi 与 Waystones、Balm 一并作为客户端和服务端强制前置。四个游戏版本各自拥有独立目标和 JAR，不复用跨补丁版本二进制。

| Minecraft | Waystones 最低 / 当前 | Balm 最低 / 当前 | Shogi 最低 / 当前 | NeoForge 最低 / 当前 | Fabric Loader / API 最低 / 当前 |
|---|---|---|---|---|---|
| 26.1 | Fabric 26.1.0.1，NeoForge 26.1.0.4 / 26.1.0.4 | Fabric 26.1.0.1，NeoForge 26.1.0.6 / 26.1.0.6 | 26.1.0.1 / 26.1.0.1 | 26.1.0.19-beta / 26.1.0.19-beta | 0.18.4 / 0.144.0；0.19.3 / 0.145.1 |
| 26.1.1 | 固定源码 26.1.1.0 / 同一二进制 | 26.1.1.1 / 26.1.1.2 | 26.1.0.1 / 26.1.0.4 | 26.1.1.8-beta / 26.1.1.15-beta | 0.18.6 / 0.145.4；0.19.3 / 0.145.4 |
| 26.1.2 | 26.1.2.1 / 26.1.2.15 | 26.1.1.2 / 26.1.2.13 | 26.1.0.1 / 26.1.2.8 | 26.1.2.0-beta / 26.1.2.97 | 0.19.1 / 0.145.4；0.19.3 / 0.155.2 |
| 26.2 | 26.2.0.1 / 26.2.0.11 | 26.2.0.1 / 26.2.0.7 | 26.2.0.1 / 26.2.0.5 | 26.2.0.0-beta / 26.2.0.66 | 0.19.3 / 0.152.1；0.19.3 / 0.158.0 |

Waystones 26.1.1 没有公开正式加载器 JAR。本项目只从官方提交 `795bb9ac93e73a0df8e5678ba6746dfbf8b055a3` 构建 `26.1.1.0`，并用仓库内 SHA 锁定补丁把 Balm、Shogi、Unbreakables、JourneyMap API、Loom 和 ModDev 输入固定到正式版或不可变时间戳。公开安装条件闭合并完成实际安装验收前，26.1.1 不进入正式上传清单。生成的上游 JAR只位于忽略的 `build/`，不提交、不嵌入、不作为本项目发行物；Build/Runtime 独立重建并要求 SHA-256 一致。

Shogi `26.1.0.4` 及后续正式加载器 JAR把公共 API作为 jar-in-jar 提供，但部分 26.1 线 Maven API坐标只有浮动快照。构建因此只从矩阵严格锁定的 Shogi 加载器 JAR确定性提取其内嵌 API用于编译和测试；真实运行继续由同一加载器 JAR提供相同字节，禁止单独解析 Shogi API快照。例外是 Minecraft 26.1：Shogi `26.1.0.4` 的加载器元数据已要求 Minecraft 26.1.1，所以 26.1 的 minimum/current 都固定到最后一个实际接受 26.1 的 `26.1.0.1`。

Runtime 门禁还排除了几组版本名看似匹配、实际不能启动的组合：NeoForge `26.1.0.1-beta` 的物理端清理器会在专用服务器初始化 Balm/Waystones 时误拒它们的客户端类型引用，因此 NeoForge 26.1 只支持已验证的 `26.1.0.19-beta` 与对应 current 前置；Balm `26.1.1.1` 要求 NeoForge 至少 `26.1.1.8-beta`且 Fabric API 至少 `0.145.4+26.1.1`；Minecraft 26.1.2 不能用仅启动 26.1.1 的 NeoForge `26.1.1.15-beta`。矩阵使用上表经运行日志确认的下限，不将这些失败组合宣称为支持。

Waystones 26.x 的传送上下文继承 Shogi 事务模型。适配层优先使用可用版本的 `tryTeleportAsync`；固定的早期 26.1.1 API只有同步入口，因此把其结果包装进相同的内部异步接口。两种路径都只使用公共 `Waystone` 接口和锁定的 Shogi context，且所有完成回调回到服务端主线程。任何规则解析失败、未知效果、非有限/负数/溢出、执行器或费用敏感字段替换都在消费与移动前关闭失败。内建 XP 点数/等级保留，其他费用不应用于玩家目的地；Shogi 的物品损坏效果转为受控延迟总额，遵守耐久开关，并在实际移动确认后原生结算一次；不是固定 1 点。规则同时包含新版 warpSettings，实际金额检查发生在原生数值转换之前，准备期间的规则重载触发重新计算。
...
5. 玩家目录、主副手、客户端 listed 展示与服务端在线 UUID解析、费用、回滚、耐久、事件取消、跨维度和失败界面行为通过对应运行验收。
...
| `neoforge/26.x` | NeoForge | 26.1–26.2 | 4 | 每个游戏版本独立 JAR |
| `fabric/26.x` | Fabric | 26.1–26.2 | 4 | 每个游戏版本独立 JAR |

总计 28 个维护目标 JAR；只有完成验收的目标才能上传，状态见 [VALIDATION.md](VALIDATION.md)。这些 JAR，全部版本号 `1.0.1`。除明确共享的 1.21.2/1.21.3 外，每份元数据只接受一个 Minecraft 小版本。
...
| 1.21.10 | 21.10.64 / 21.10.64 | 21.10.1 / 21.10.5 | 21.10.3 / 21.10.11 | context/hand 断点；21.10.64 修复 userdev joined patches |
...
26.x 另核对：全部原生 WarpStoneItem（包括该版本存在的材质变体）、完整默认规则、分页与滚动列表断点、新版绘制接口、后台完成的准备任务、目标移动后的重新准备，以及早期 Shogi 的副手上下文继承。相关 Mixin 必须用加载器测试环境或实际游戏验证，普通 Java 单测不能证明已注入。
